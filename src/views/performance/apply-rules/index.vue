<template>
  <div class="apply-container">
    ...
    <el-tabs v-model="activeTab" class="apply-tabs">
      ...
      <!-- Tab 3: 联动说明 -->
      <el-tab-pane label="联动说明" name="help">
        <el-card class="card">
          <div class="help-intro">
            <div class="help-title">💡 绩效结果应用联动 — 如何让绩效驱动后续决策</div>
            <div class="help-desc">绩效结果不是目的，而是驱动薪酬、培训、晋升等人才决策的输入。本页配置"条件 → 动作"规则，让绩效自动触发后续环节。</div>
          </div>
          ...
          <div class="help-grid">
            <div v-for="(c, k) in RULE_CATEGORY_MAP" :key="k" class="help-card" :style="{ borderLeft: `4px solid ${c.color}` }">
              ...
              <div class="help-cat-link" @click="goCategory(k)">查看此类规则 →</div>
            </div>
          </div>
        </el-card>
      </el-tab-pane>
...
defineOptions({ name: 'PerformanceApplyRules' })
