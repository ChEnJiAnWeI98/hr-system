      ...
      <div class="table-container">
        ...
        >
          ...
          <el-table-column prop="status" label="发放状态" min-width="8%">
            <template #default="{ row }">
              <el-tag v-if="row.payslipStatus === 1" type="warning">未发放</el-tag>
          ...
          <el-table-column label="操作" min-width="25%" fixed="right">
            <template #default="{ row }">
              ...
              <el-button link @click="handleAdjust(row)" :disabled="row.payslipStatus === 2">
                调整工资
              </el-button>
              <el-button link type="primary" @click="handleSendPayslip(row)" :disabled="row.payslipStatus === 1">
                发放工资条
              </el-button>
...
import { getSalaryRecordList } from '@/api/salary'
...
const loadTableData = async () => {
  try {
    const res = await getSalaryRecordList({
      ...
      payslipStatus: queryParams.status,
