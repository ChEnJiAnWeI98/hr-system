      ...
      <el-form :model="queryParams">
        <div class="filter-form-content">
          ...
          <el-form-item label="员工状态">
            <el-select v-model="queryParams.employeeStatus" placeholder="请选择员工状态" style="width: 150px" clearable>
    ...
    <el-card class="data-card">
      ...
      <div class="table-container">
        ...
        >
          ...
          <el-table-column prop="employeeStatus" label="员工状态" min-width="8%">
            <template #default="{ row }">
              <el-tag v-if="row.employeeStatus === 1" type="success">在职</el-tag>
              <el-tag v-else-if="row.employeeStatus === 2" type="warning">试用期</el-tag>
              <el-tag v-else-if="row.employeeStatus === 3" type="info">已离职</el-tag>
            </template>
          </el-table-column>
      ...
      >
        ...
        <el-form-item label="调薪后薪资" prop="afterSalary">
          ...
          <AISalarySanityInline
            ...
            :required-fields="[addForm.employeeName, addForm.afterSalary, addForm.beforeSalary]"
            :target-name="addForm.employeeName || ''"
          />
        </el-form-item>
...
// 员工列表：从员工池动态读取，只保留在职员工（regular/probation/transferring）
import { EMPLOYEES } from '@/mock/core/employeePool'
const employeeList = computed(() =>
  EMPLOYEES
    .filter((e) => e.status === 'regular' || e.status === 'probation' || e.status === 'transferring')
    .map((e) => ({
      id: e.id,
      code: e.employeeNo,
      name: e.nameZh,
      department: e.orgName,
...
const buildSalaryCheckInput = () => {
  return `员工：${addForm.employeeName || '-'}（部门：${addForm.departmentName || '-'}）\n调薪类型：${addForm.adjustmentType || '-'}\n当前薪资：¥${addForm.beforeSalary || '-'}\n调整后：¥${addForm.afterSalary || '-'}\n调整原因：${addForm.reason || '-'}\n备注：请诊断该调薪方案是否偏离岗位带宽、是否触发与同岗位/Leader 的倒挂，并给出调整建议。`
}
...
const handleReset = () => {
  ...
  queryParams.employeeStatus = null
  queryParams.page = 1
  getList()
}
