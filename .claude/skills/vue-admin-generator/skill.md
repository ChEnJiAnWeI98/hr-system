...
description: >
  You MUST use this skill when users ask to create, generate, or build an admin dashboard or management system.
  Triggers: (1) "创建管理后台" "生成后台系统" "搭建管理系统" "后台框架",
  (2) "搭建xx系统管理后台" "搭建xx系统后台框架" "xx系统管理后台",
  (3) "搭建xx系统" (when context indicates admin system),
  (4) Users describe admin system requirements (login page, layout, menu structure, theme colors),
  (5) Users provide admin system design mockups or sketches for analysis,
  (6) "管理后台项目" "后台管理系统" "Admin项目",
  (7) Need an admin frontend project with Mock data (no backend required).
  Uses Vue3 + Element Plus + Vite + TypeScript + Mock.js stack for complete web admin systems.
---
